...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (login_user('admin', $email, $password, $error)) {
...
<head>
    ...
    <title>Admin Login | EduTranscript</title>
...
<script>
(() => {
    const btns = document.querySelectorAll('button[data-theme]');
    function apply(n){
        document.body.classList.remove('bg-image');
        document.documentElement.classList.remove('theme-dark');
        if(n==='image') document.body.classList.add('bg-image');
        if(n==='dark') document.documentElement.classList.add('theme-dark');
        localStorage.setItem('edutranscript_theme', n);
    }
